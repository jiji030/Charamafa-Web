        ...
        <div id="report-content" class="p-6">
          ...
          <div v-else>
            <div class="mb-8">
              ...
              <p class="text-center text-gray-600 mb-4">Charito Mahayag Farmers Association</p>
            ...
            <div v-for="tsGroup in Object.values(membersByTS)" :key="tsGroup.ts_id" class="mb-12 page-break">
              <div class="bg-gray-900 text-white p-4 mb-6 rounded-lg shadow-lg border-l-4 border-green-500">
                <div class="flex justify-between items-start">
                  <div>
                    <p class="text-3xl font-bold">TS {{ tsGroup.ts_no }}</p>
                    <p class="text-lg mt-1">{{ tsGroup.landmark || 'No Landmark' }}</p>
                  </div>
                  <div class="text-right text-sm opacity-75">
                    <p>Territorial Section</p>
                  </div>
                </div>
                <div class="border-t border-gray-600 mt-3 pt-3">
                  <p class="text-xs">Members: {{ tsGroup.members.length }}</p>
                </div>
              </div>
              ...
              <table class="w-full text-base border-collapse mb-8">
                <thead>
                  <tr class="bg-green-100 border-b-2 border-gray-800">
                    ...
                    <th class="border px-3 py-2 text-center font-semibold text-sm">Prev CUM</th>
                    <th class="border px-3 py-2 text-center font-semibold text-sm">Present CUM</th>
                    <th class="border px-3 py-2 text-center font-semibold text-sm">Consumption</th>
                    ...
                    <th class="border px-3 py-2 text-center font-semibold text-sm">Prev Reading</th>
                    <th class="border px-3 py-2 text-center font-semibold text-sm">Present Reading</th>
                ...
                <tbody>
                  <tr v-for="member in tsGroup.members" :key="member.member_id" class="border-b hover:bg-gray-50 text-base"
                    ...
                    <td class="border px-3 py-2 text-center text-sm">{{ member.prev_CUM_consumption || 0 }}</td>
                    <td class="border px-3 py-2 text-center text-sm">{{ member.present_CUM_consumption || 0 }}</td>
                    <td class="border px-3 py-2 text-center text-sm font-semibold">{{ member.cum_consumption || 0 }}</td>
                    ...
                    <td class="border px-3 py-2 text-center text-sm">{{ member.prev_meter_reading || '-' }}</td>
                    <td class="border px-3 py-2 text-center text-sm">{{ member.present_meter_reading || '-' }}</td>
...
const tabs = [
  ...
  { label: 'Print Water Report', path: '/president/water-report' }
]
...
const loadReportData = async () => {
  ...
  try {
    ...
    data.forEach(member => {
      const tsId = member.ts_Id || member.ts_id || 'Unknown'
      if (!membersByTS.value[tsId]) {
        membersByTS.value[tsId] = {
          ts_id: member.ts_Id || member.ts_id,
          ts_no: member.ts_no || '',
          landmark: member.landmark || '',
          members: []
        }
      }
      membersByTS.value[tsId].members.push(member)
    })
  } catch (err) {
  ...
  const htmlContent = `
    ...
    <body>
      ...
      <div class="report-meta">
        <p>Charito Mahayag Farmers Association</p>
