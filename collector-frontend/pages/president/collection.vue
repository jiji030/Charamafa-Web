      ...
      <div class="bg-white rounded-lg shadow overflow-hidden">
        <div class="p-6 border-b flex justify-between items-center">
          <h2 class="text-xl font-bold text-gray-800">Payment Collection (Monthly)</h2>          <div class="flex gap-4 items-center">
            ...
            >
              ...
              <option value="collector_name">Collector</option>
            </select>
        ...
        <div id="printable-collection" class="overflow-x-auto">
          <table class="w-full">            <thead class="bg-green-50">
              <tr>
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase">Billing Period</th>
            ...
            <tbody class="divide-y divide-gray-200">
              <tr v-for="collection in filteredCollections" :key="collection.collection_id" class="hover:bg-gray-50">
                ...
                <td class="px-6 py-4 text-sm font-semibold text-gray-900 text-right">₱{{ formatNumber(collection.total_collection) }}</td>
            ...
            <tfoot class="bg-green-50">
              <tr>
                ...
                <td class="px-6 py-4 text-sm font-bold text-green-700 text-right">₱{{ formatNumber(totalCollections) }}</td>
...
const filteredCollections = computed(() => {
  ...
  filtered = [...filtered].sort((a, b) => {
    ...
    if (sortBy.value === 'date') {
      // Sort by billing_month (YYYY-MM format)
      aVal = a.billing_month
      bVal = b.billing_month
    } else if (sortBy.value === 'amount') {
      aVal = a.total_collection
      bVal = b.total_collection
    } else if (sortBy.value === 'collector_name') {
      aVal = a.collector_name.toLowerCase()
      bVal = b.collector_name.toLowerCase()
    }
